...
class PATNetwork(nn.Module):
    ...
    def Transformation_Feature(self, query_feats, support_feats, prototypes_f, prototypes_b):
        ...
        for idx, feature in enumerate(support_feats):
            ...
            eps = 1e-6
            #随着训练的进行，锚层参考特征会逐渐学习到如何表示支持图像和查询图像之间的特征关联，从而帮助模型跨域泛化
            if idx <= 3:
    ...
    def finetune_reference(self, batch, query_mask, nshot):
        ...
        for s_idx in range(nshot):
            ...
            for idx, feature in enumerate(prototypes_sf):
                C = torch.cat((prototypes_sb[idx].unsqueeze(1), prototypes_sf[idx].unsqueeze(1)), dim=1)
                eps = 1e-6
                if idx <= 3:
                    R = self.reference_layer1.weight.expand(C.shape)
                    power_R = ((R * R).sum(dim=2, keepdim=True)).sqrt()
                    ...
                    support_mapped = torch.matmul(P, feature)
                    support_mapped = support_mapped.squeeze(-1)

                    prototypes_qf[idx] = prototypes_qf[idx].unsqueeze(-1)
                    query_mapped = torch.matmul(P, prototypes_qf[idx])
                    query_mapped = query_mapped.squeeze(-1)
                    kl += F.kl_div(query_mapped.softmax(dim=-1).log(), support_mapped.softmax(dim=-1), reduction='sum')
                elif idx <= 9:
                    R = self.reference_layer2.weight.expand(C.shape)
                    power_R = ((R * R).sum(dim=2, keepdim=True)).sqrt()
                    R = R / (power_R + eps)
                    ...
                    query_mapped = torch.matmul(P, prototypes_qf[idx])
                    query_mapped = query_mapped.squeeze(-1)
                    kl += F.kl_div(query_mapped.softmax(dim=-1).log(), support_mapped.softmax(dim=-1),
                                   reduction='sum')
                elif idx <= 12:
                    R = self.reference_layer3.weight.expand(C.shape)
                    power_R = ((R * R).sum(dim=2, keepdim=True)).sqrt()
